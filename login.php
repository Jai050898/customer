...
if(isset($_REQUEST['hid_type']) && $_REQUEST['hid_type'] == 'Reg')
{
	...
	if($cnt != 0)
	{
		if($UsrDet[0]['login_status'] == 'A' && $UsrDet[0]['status'] == 'A' && $UsrDet[0]['attempts'] < 5)
		{
			...
			//Code for YouSendIt
			$yousenditPath = SITEPATH . '/yousendit/sample';
			$yousenditIncludePath = SITEPATH . '/yousendit/sample/include';

			if (!file_exists($yousenditPath . '/Authentication.php') || !file_exists($yousenditIncludePath . '/RestTransport.php') || !file_exists($yousenditIncludePath . '/ResponseEntities.php')) {
				trigger_error('YouSendIt files not found at expected path: ' . $yousenditPath, E_USER_WARNING);
			} else {
				require_once($yousenditPath . '/Authentication.php');
				require_once($yousenditIncludePath . '/RestTransport.php');
				require_once($yousenditIncludePath . '/ResponseEntities.php');
			}
		...
		// 	$d = date("Y-m-d H:i:s");
		// 	$UpLogin		= $Gen->ExecQuery("UPDATE tbl_users SET attempts  = attempts+1, locked_date = '".$d."' WHERE user_name = '".$_REQUEST['Log']['user_name']."'");
		// }
	}
	else
	{
