...
error_reporting(E_ALL);
    ...
    {//echo "<pre>";print_r($permission);exit;
        ...
        foreach($categories as $index => $category)
        {
            ...
            {               
                $menu = array();               
                $ret .= '<li style="list-style-type:none;"><input type="checkbox" id="parent_id'.$category['parent_id'].'" name="menu_permit[]" value="'.$category['id'].'"';
...
//echo "<prE>";print_r($_REQUEST);exit;
    ...
    $table  = "user_menu U";
    $fields = "U.*, (SELECT count(distinct U2.id) from user_menu U2 WHERE U2.parent_id = U.id) as ChildCount";
    
    $menuArray  = array();
    //$menuArray  = $Gen->GetSelWhere("user_menu", "*", " 1=1 AND status = 'A' ORDER BY parent_id, menu_order");
    $menuArray  = $Gen->GetSelWhere($table, $fields, " 1=1 AND status = 'A' ORDER BY parent_id, menu_order");
