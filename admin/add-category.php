...
if(isset($_REQUEST['cat_id']) && $_REQUEST['cat_id'] != "" && $_REQUEST['hid_key']=='Post')
{
	...
	$UpOverview 				= $Gen->UpdateQry("tbl_categories",$PrFields,"cat_id = ".$_REQUEST['cat_id']);
...
if(isset($_REQUEST['hid_key']) && $_REQUEST['hid_key']=='Post')
{
	//echo '<pre>';print_r($_REQUEST);exit;
	$PrFields = $_REQUEST['Log'];
	$res = $usr->SelectQuery("SELECT max(showorder) as maxid FROM tbl_categories");
	$PrFields['showorder'] = $res[0]['maxid']+1;
	//echo "<pre>";print_r($PrFields);exit;
	$ins 						= $Gen->InsertQry('tbl_categories',$PrFields);
	header("Location:".SITEURL.'/admin/manage-categories.php');
}

$smarty->display('add-category.tpl');
?>