								...
								<td align="left" valign="top" colspan="2">
								  <table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
									<tr>
										...
										<th width="10%" align="left"><a href="javascript: setClientSort('MIS_Year','{$sortioption}',document.ManageMISVehicles);">Year</a> {if $smarty.request.sortby eq 'MIS_Year'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle"  />{/if}  </th>
										<th width="12%" align="left"><a href="javascript: setClientSort('MIS_Make','{$sortioption}',document.ManageMISVehicles);">Make </a> {if $smarty.request.sortby eq 'MIS_Make'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle" />{/if}</th>
										<th width="12%" align="left"><a href="javascript: setClientSort('MIS_Model','{$sortioption}',document.ManageMISVehicles);">Model </a> {if $smarty.request.sortby eq 'MIS_Model'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle" />{/if}</th>
									...
									{foreach item=item name=item from=$Users}
										<tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
											<td align="left"><input type="checkbox" name="chkall[]" id="chkall" value="{$item.MIS_Vehicle_ID}"/></td>
											<td align="left">{$item.MIS_Year}</td>
											<td align="left"> {$item.MIS_Make}</td>
											<td align="left"> {$item.MIS_Model}</td>
											<td align="left"><a href="{$siteurl}/admin/view-mis-vehicle.php?vid={$item.MIS_Vehicle_ID}">{$item.MIS_Vin}</a></td>
											<td align="left">{$item.MIS_Body}</td>
											<td align="left" >{$item.MIS_Engine}</td>
										</tr>
									{foreachelse}
										<tr>
