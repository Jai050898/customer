								  ...
								  <table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
									...
									{foreach item=item name=item from=$Users}
                                                                            <tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
                                                                                    ...
                                                                                    <td align="left"><a href="{$siteurl}/admin/view-xml-customer.php?user_id={$item.cust_id}">{if $item.fullname neq ''}{$item.fullname}{else}-N/A-{/if}</a></td>
                                                                                    <td align="left" style="padding-right:15px;"> {$item.name}</td>
                                                                                    <td align="left">{$item.rec_text|nl2br}</td>
                                                                                    <td align="left" >{$item.rec_date|date_format:"%m/%d/%y"}</td>
                                                                                    <td align="left" >{$item.dt_follow_update|date_format:"%m/%d/%y"}</td>
                                                                            </tr>
									{foreachelse}
                                                                            <tr>
