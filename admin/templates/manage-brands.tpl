		...
		<input type="hidden" name="sortoption" value="{$smarty.request.sortoption}" />
			...
			<tr>
			 ...
			 <tr>
			  ...
			  <td align="right"><a href="{$siteurl}/admin/add-brand.php">Add Brands</a></td>
			</tr>
			 <tr>
			  ...
			  <select name="cid" id="cid" class="searchinput">
									...
									<option value="{$item.id}" {if $smarty.request.cid eq $item.id} selected="selected"{/if}>{$item.name}</option>
									{/foreach}
								</select>&nbsp;&nbsp;
			...
			<tr>
			  <td align="left" valign="top" colspan="2"><table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
				<tr>
				  ...
				  <th width="40%" align="left"><a href="javascript: setClientSort('brand_name','{$sortioption}',document.ManageCat);">Name</a> {if $smarty.request.sortby eq 'brand_name'}<img src="../images/{$sortimoption}.gif" alt="" align="absmiddle"  />{/if}  </th>
				...
				<tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
				  <td align="left"><input type="checkbox" name="chkall[]" id="chkall" value="{$item.brand_id}"/></td>
				  <td align="left">{$item.brand_name|stripslashes}</td>
				  <td align="left">{if $item.catname neq ""}{$item.catname|stripslashes}{else}NA{/if}</td>
				  <td align="left">{if $item.export eq "Y"}<font color="#00CC33">YES</font>{else}<font color="#FF0000">NO</font>{/if}</td>
				  ...
				  <td align="left"><a href="{$siteurl}/admin/add-brand.php?brand_id={$item.brand_id}" class="link023">Edit</a></td>
				</tr>
				{foreachelse}
				<tr>
