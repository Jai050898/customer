	  ...
	  <div class="ad_textsp">
		...
		<input type="hidden" name="sortoption" value="{$smarty.request.sortoption}" />
			...
			<tr>
			 ...
			 <td align="right"><a href="{$siteurl}/admin/add-portfolio-category.php">Add Category</a></td>
			</tr>
			 <!--<tr>
			...
			<tr>
			  <td align="left" valign="top" colspan="2"><table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
				<tr>
				  ...
				  <th width="40%" align="left"><a href="javascript: setClientSort('cat_name','{$sortioption}',document.ManageCat);">Category Name</a> {if $smarty.request.sortby eq 'cat_name'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle"  />{/if}  </th>
				  <!--<th width="15%" align="left">Status </th>-->
				  <th width="15%" align="left"><a href="javascript: setClientSort('created_date','{$sortioption}',document.ManageCat);">Crated Date </a> {if $smarty.request.sortby eq 'created_date'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle" />{/if}</th>
				...
				<tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
				  <td align="left"><input type="checkbox" name="chkall[]" id="chkall" value="{$item.cat_id}"/></td>
				  <!--<td align="left">{if $smarty.foreach.item.first}<a href="{$siteurl}/admin/manage-categories.php?act=down&id={$item.cat_id}&order={$item.showorder}"><img src="{$siteurl}/images/arrow_down.png" /></a>{elseif $smarty.foreach.item.last}<a href="{$siteurl}/admin/manage-portfolio-categories.php?act=up&id={$item.cat_id}&order={$item.showorder}"><img src="{$siteurl}/images/arrow_up.png" /></a>{else}<a href="{$siteurl}/admin/manage-categories.php?act=down&id={$item.cat_id}&order={$item.showorder}"><img src="{$siteurl}/images/arrow_down.png" /></a>&nbsp;&nbsp;<a href="{$siteurl}/admin/manage-categories.php?act=up&id={$item.cat_id}&order={$item.showorder}"><img src="{$siteurl}/images/arrow_up.png" /></a>{/if}</td> -->
				  <!--<td align="left">{$item.showorder}</td> -->
				  <td align="left"><!--<a href="{$siteurl}/admin/view-category.php?cat_id={$item.cat_id}">{$item.cat_name}</a>--><a href="#">{$item.cat_name}</a>
                                  </td>
			<!--	  <td align="left">{if $item.status eq "A"}<font color="#00CC33">Active</font>{elseif $item.status eq "I"}<font color="#0000FF">InActive</font>{elseif $item.status eq "P"}<font color="#00EEFF">Pending</font>{else}<font color="#FF0000">Deleted</font>{/if}</td>-->
				  ...
				  <td align="left"><a href="{$siteurl}/admin/add-portfolio-category.php?cat_id={$item.cat_id}" class="link023">Edit</a></td>
				</tr>
				{foreachelse}
				<tr>
