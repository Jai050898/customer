      ...
      <div id="admin_head">Marketing Budget</div>
	   ...
	   <div class="ad_textsp">
			...
			{foreach item=Citem name=Citem from=$item1.Arr}
				<td>{if $Citem.amount neq ""}$ {$Citem.amount|number_format:2:".":","}{else} NA {/if}</td>
				<td>{if $Citem.actual_amount neq ""}$ {$Citem.actual_amount|number_format:2:".":","}{else} NA {/if}</td>
			{foreachelse}
				<td>NA</td>
				<td>NA</td>
			{/foreach}
			...
			{foreach item=Calitem name=Calitem from=$Cal}
				<td><strong>$ {$Calitem.gsales|number_format:2:".":","}</strong></td>
				<td><strong>$ {$Calitem.gsales|number_format:2:".":","}</strong></td>
			{/foreach}
			</tr>
			<tr style="background-color:#f3f3f3;">
			...
			{foreach item=Calitem name=Calitem from=$Cal}
				<td><strong>$ {$Calitem.amount|number_format:2:".":","}</strong></td>
				<td><strong>$ {$Calitem.actualamount|number_format:2:".":","}</strong></td>
			{/foreach}
			</tr>
			<tr style="background-color:#f3f3f3;">
