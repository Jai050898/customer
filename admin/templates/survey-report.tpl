			...
			<tr>
			  <td align="left" valign="top" colspan="2"><table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
				...
				<tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
				  <td align="left"><a href="{$siteurl}/admin/view-survey.php?user_id={$smarty.request.user_id}&survey_id={$item.cat_id}">{$item.cat_name}</a>&nbsp;&nbsp;{if $item.completed eq "Y"}(<font color="#0000FF">Completed&nbsp;&nbsp;<a href="export_survey.php?uid={$smarty.request.user_id}&sid={$item.cat_id}">Export Survey</a></font>){/if}</td>
				</tr>
				{foreachelse}
				<tr>
