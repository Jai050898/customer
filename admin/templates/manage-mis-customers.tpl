							...
							<tr>
							  <td  colspan="2">
								...
								<select name="user_id" id="user_id" class="searchinput">
									...
									<option value="{$item.user_id}" {if $smarty.request.user_id eq $item.user_id} selected="selected"{/if}>{$item.company_name|stripslashes}</option>
									{/foreach}
								</select>&nbsp;&nbsp;
								...
								<td align="left" valign="top" colspan="2">
								  <table width="100%" cellpadding="0" cellspacing="0" class="admin_table">
									<tr>
										...
										<th width="17%" align="left"><a href="javascript: setClientSort('MIS_city','{$sortioption}',document.ManageMISCustomers);">City </a> {if $smarty.request.sortby eq 'MIS_city'}<img src="images/{$sortimoption}.gif" alt="" align="absmiddle" />{/if}</th>
									...
									{foreach item=item name=item from=$Users}
										<tr style="background-color:{cycle values='#ffffff,#f3f3f3'};">
											<td align="left"><input type="checkbox" name="chkall[]" id="chkall" value="{$item.MIS_cust_ID}"/></td>
											...
											<td align="left" style="padding-right:15px;"> {$item.MIS_city}</td>
											<td align="left">{$item.MIS_state}</td>
											<td align="left">{$item.MIS_LifetimeVisits}</td>
											<td align="left" ><a href="{$siteurl}/admin/manage-mis-vehicles.php?user_id={$item.MIS_cust_ID}">{$item.Vcount}</a></td>
											<td align="left" ><a href="{$siteurl}/admin/manage-mis-ros.php?user_id={$item.MIS_cust_ID}">{$item.Rcount}</a></td>
										</tr>
									{foreachelse}
										<tr>
