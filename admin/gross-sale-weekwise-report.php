    ...
    for($i=$endingYear; $i <= $currentYear; $i++) {
        ...
        for($j=1;$j<=12;$j++) {
            ...
            for($k=1;$k<=$maxday;$k++) {
                ...
                $roArray    = array();
                $roArray    = $Gen->GetSelWhere('XML_ro', 'sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as gross', "1=1 AND company_id = '".$_REQUEST['user_id']."' AND DATE(transaction_date) = '".$monthArray[$k]['date']."'");
        ...
        /*
        for($j=1;$j<=12;$j++) {
            for($k=1;$k<=$allArray[$i]['MonthAry'][$j]['MaxDay'];$k++) {
                for($l=1;$l<=$allArray[$i]['MonthAry'][$j]['MaxDay'];$k++) {
                    $averageROVal = $allArray[$i]['MonthAry'][$j]['DaysAry'][$l]['avgRO'];
                    
                    
                    if($averageROVal != 0 && $averageROVal < $lowest30thVal) {
                        $allArray[$i]['MonthAry'][$j]['DaysAry'][$l]['color'] = "RED";
                    } elseif($averageROVal != 0 && $averageROVal > $highest30thVal) {
                        $allArray[$i]['MonthAry'][$j]['DaysAry'][$l]['color'] = "BLUE";
                    } else {
                        $allArray[$i]['MonthAry'][$j]['DaysAry'][$l]['color'] = "BLACk";
                    }
                }
                
                if($i == 2014) {
                    echo "lowest-->".$lowest30thVal."<br>Highest-->".$highest30thVal;
                    echo "<pre>";print_r($newArray);
                    echo "<pre>";print_r($allArray[$i]['MonthAry'][$j]['DaysAry']);exit;
        ...
        //echo "   Highest->".$highest30thVal;
        //echo "<br>Lowest->".$lowest30thVal;
        //echo "<pre>".$i;print_r($newArray);//exit;
