...
$Fields		= "date";
$Where     = " customer_id = '".$_REQUEST['user_id']."' AND type= 'B' GROUP BY date ";
...
//$Images = $Gen->GetSelWhere("tbl_thumbnails","date"," customer_id = '".$_REQUEST['user_id']."' AND type= 'B' GROUP BY date ORDER BY date DESC");
for($i=0;$i<count($Images);$i++)
{
	$items = $Gen->GetSelWhere("tbl_thumbnails","image"," customer_id = '".$_REQUEST['user_id']."'  AND type= 'B' AND date = '".$Images[$i]['date']."'");
