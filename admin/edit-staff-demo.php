        ...
        $id  = addslashes(trim($_POST['hid_key']));
          ...
          if(isset($id)) {
           ...
           $PrFields['user_name']  = addslashes(trim($_POST['Log']['user_name']));
            $PrFields['password']  = addslashes(trim($_POST['Log']['password']));
            ...
            elseif(filter_var($PrFields['email'], FILTER_VALIDATE_EMAIL) == '') {$error  = "Invalid Email Format!!!";}
            ...
            elseif($PrFields['city']   == '')	
                    $error  = "City can not be empty";
                        ...
                        $UnameCnt = $Gen->TotalRows('tbl_users',"user_name = '".$PrFields['user_name']."' AND user_id != '".$_REQUEST['user_id']."' ");
                                    //echo $UnameCnt;
                                                if($UnameCnt == 0){
