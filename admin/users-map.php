...
$AccDetarr	= $Gen->GetSelWhere($Table,$Fields," user_id = ".$_REQUEST['user_id']);
...
{
	$where = "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_coordinates != ''";
...
else
{
	...
	//Map data
	$MapWhere		= "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_coordinates != '' ORDER BY MIS_FirstVisited DESC LIMIT 0,1000";
	...
	if($_REQUEST['lastyear'] == "")
	{
		...
		$MapLWhere		= "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_FirstVisited > '".$Maplast."' AND MIS_FirstVisited < '".$Maplast3month."' AND MIS_coordinates != '' ORDER BY MIS_FirstVisited DESC LIMIT 0,1000";
		$MapLFields		= "*";
	...
	if($_REQUEST['lastm'] == "")
	{
		...
		$MapLMWhere		= "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_FirstVisited > '".$Maplastmonth."' AND MIS_coordinates != '' ORDER BY MIS_FirstVisited DESC LIMIT 0,1000";
		$MapLMFields		= "*";
	...
	if($_REQUEST['last3m'] == "")
	{
		...
		$Mapmonth = date('m') -1 ;
		$Maplastmonth = date("Y-m-d", mktime(0, 0, 0, 1, $Mapmonth, $Mapyear));
		$MapL3MWhere		= "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_FirstVisited > '".$Maplast3month."' AND MIS_FirstVisited < '".$Maplastmonth."' AND MIS_coordinates != '' ORDER BY MIS_FirstVisited DESC LIMIT 0,1000";
		$MapL3MFields		= "*";
	...
	if($_REQUEST['totcust'] == "")
	{
		...
		$MapTWhere		= "1=1 AND market_cust_id = '".$_REQUEST['user_id']."' AND MIS_FirstVisited < '".$last."' AND MIS_coordinates != '' ORDER BY MIS_FirstVisited DESC LIMIT 0,1000";
		$MapTFields		= "*";
		$MapTRes		= $usr->GetSelWhere($Table,$MapTFields,$MapTWhere);
		for($k=0;$k<count($MapTRes);$k++)
		{
	...
	if($_REQUEST['comphid'] == "all")
	{
		...
		$MapTWhere		= "1=1 AND user_id = '".$_REQUEST['user_id']."' AND status = 'A'";
		$MapTFields		= "*";
		$MapTRes		= $usr->GetSelWhere($Table,$MapTFields,$MapTWhere);
		for($k=0;$k<count($MapTRes);$k++)
		{
