<?php
    ...
    foreach($custAry as $key=>$customer) {
        ...
        $Whr    = " 1=1 AND company_id = '".$_REQUEST['user_id']."'";
        ...
        if($visitCnt != 0) {
            $customerArray[$key]['avgRO']        = $totalGrossSale / $visitCnt;
        } else {
            $customerArray[$key]['avgRO']        = 0;
        }
        $customerArray[$key]['VisitArray']   = $resVisitAry;
        
        if($visitCnt < 5) {
            for($i=1;$i<=(5-$visitCnt);$i++) {
                $customerArray[$key]['VisitArray'][]['grossSales'] = '-1';
            }
        }
   ...
   //exit;
    ...
    $srcpath 	= "user_id=".$_REQUEST['user_id']."&sortby=".$_REQUEST['sortby']."&sortoption=".$_REQUEST['sortoption']."&fdate=".$_REQUEST['fdate']."&tdate=".$_REQUEST['tdate']."&page=";
    ...
    $smarty->display('customer-xml-visit-spending.tpl');
?>
