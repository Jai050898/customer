...
if(isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != "")
{
	...
	$user	= $usr->GetSelWhere("tbl_writer_users",$Fields,$Where);
...
if(isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != "" && $_REQUEST['hid_key']=='Post')
{
	...
	$UpOverview 				= $Gen->UpdateQry("tbl_writer_users",$PrFields,"user_id = ".$_REQUEST['user_id']);
	header("Location:".SITEURL.'/admin/manage-writerusers.php');
	exit;
}
...
if(isset($_REQUEST['hid_key']) && $_REQUEST['hid_key']=='Post')
{
	...
	$PrFields = $_REQUEST['Log'];
	$PrFields['state'] = $_REQUEST['state'][0];
	$ins 						= $Gen->InsertQry('tbl_writer_users',$PrFields);
	if($ins)
	{
		//Code to send mail
		...
		$result		.='<br> Link : http://www.autorepairmarketing.com/customer/writer';
...
$smarty->display('add-writeruser.tpl');
?>