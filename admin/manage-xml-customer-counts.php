<?php
    ...
    // To FETCH TOTAL COUNTS OF CUSTOMERS
    
    //In Last 6 Months
    $countsArray    = array();
    $tot6Where     = $Where1." AND transaction_date > DATE_SUB(now(), INTERVAL 6 MONTH) AND status = 'A'";
    $countsArray['6MonthTotal'] = $usr->TotalRows("XML_ro",$tot6Where);
    
    // in 6-12Months
    $tot6to12Where     = $Where1." AND (transaction_date BETWEEN (CURRENT_DATE() - INTERVAL 12 MONTH) AND (CURRENT_DATE() - INTERVAL 6 MONTH)) AND status = 'A'";
    $countsArray['6to12MonthTotal'] = $usr->TotalRows("XML_ro",$tot6to12Where);
    
    // In 12-18Months
    $tot12to18Where     = $Where1." AND (transaction_date BETWEEN (CURRENT_DATE() - INTERVAL 18 MONTH) AND (CURRENT_DATE() - INTERVAL 12 MONTH)) AND status = 'A'";
    $countsArray['12to18MonthTotal'] = $usr->TotalRows("XML_ro",$tot12to18Where);
    
    // In 18-24 MOnths
    $tot18to24Where     = $Where1." AND (transaction_date BETWEEN (CURRENT_DATE() - INTERVAL 24 MONTH) AND (CURRENT_DATE() - INTERVAL 18 MONTH)) AND status = 'A'";
    $countsArray['18to24MonthTotal'] = $usr->TotalRows("XML_ro",$tot18to24Where);
    
    // In 24-36 Months
    $tot24to36Where     = $Where1." AND (transaction_date BETWEEN (CURRENT_DATE() - INTERVAL 36 MONTH) AND (CURRENT_DATE() - INTERVAL 24 MONTH)) AND status = 'A'";
    $countsArray['24to36MonthTotal'] = $usr->TotalRows("XML_ro",$tot24to36Where);

    // In 36-48 Months
    $tot36to48Where     = $Where1." AND (transaction_date BETWEEN (CURRENT_DATE() - INTERVAL 48 MONTH) AND (CURRENT_DATE() - INTERVAL 36 MONTH)) AND status = 'A'";
    $countsArray['36to48MonthTotal'] = $usr->TotalRows("XML_ro",$tot36to48Where);
    ...
    $srcpath 	="user_id=".$_REQUEST['user_id']. "&sortby=".$_REQUEST['sortby']."&sortoption=".$_REQUEST['sortoption']."&page=";
    ...
    $smarty->assign('countsAry', $countsArray);
    $smarty->assign('customer', $customerArray);
    $smarty->display('manage-xml-customer-counts.tpl');
?>
