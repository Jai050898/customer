<?php
    ...
    for($i=$currentYear, $j =0; $i > $lastYear; $i--,$j++){
        ...
        $custTotalClass = '';
        $custArray  = array();
        $Where1  = "1=1 AND company_id = '".$_REQUEST['user_id']."' AND YEAR(reg_date) = '".$i."'";
        $fields1 = "cust_id";
        ...
        */
        ...
        $vehicleArray    = array();
        $Where3	= "1=1 AND company_id = '".$_REQUEST['user_id']."' AND YEAR(vreg_date)= '".$i."'";
        ...
        $Where4	= "1=1 AND company_id = '".$_REQUEST['user_id']."' AND YEAR(transaction_date)= '".$i."'";
       ...
       // $totalROB4Discount  = $totalROB4Discount + $allArray['grossSales'][$j];
        if($allArray['grossRos'][$j] != 0) {
            $allArray['averageROB4Discount'][$j]    = $grossSaleArray['0']['grossSales'] / $allArray['grossRos'][$j];
        } else {
            $allArray['averageROB4Discount'][$j] = 0;
        }
        //$allArray['averageROB4Discount'][$j] = $averageROB4Discount;


        // Total RO After Discount
        $allArray['roAfterDiscount'][$j] = $grossSaleArray['0']['grossSales'] - $allArray['discount'][$j];
        
        //$roAfterDiscount    = $grossSaleArray['0']['grossSales'] - $allArray['discount'][$j];
        //$totalROAfterDiscount  = $totalROAfterDiscount + $roAfterDiscount;
        if($allArray['grossRos'][$j] != 0) {
            $allArray['averageROAfterDiscount'][$j] = $allArray['roAfterDiscount'][$j]/ $allArray['grossRos'][$j];        
        } else {
            $allArray['averageROAfterDiscount'][$j] = 0;
        }
        ...
        //Gross Parts Sales 
        $partsArray    = array();
        $Where6 = "1=1 AND company_id = '".$_REQUEST['user_id']."' AND YEAR(transaction_date)= '".$i."'";
        ...
        // Gross Labor Sales
        $laborArray    = array();
        $Where5	= "1=1 AND company_id = '".$_REQUEST['user_id']."' AND YEAR(transaction_date)= '".$i."'";
