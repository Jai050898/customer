...
{	
	...
	$upid	= $usr->UpdateQry('tbl_annual_revenue',$upar,"id IN(".$_REQUEST['hid_id'].")");
}
$Where		= "1=1 AND status != 'D'";
//echo "<pre>";print_r($_REQUEST);exit;
if(isset($_REQUEST['keyword']) && $_REQUEST['keyword']!='')
{
	$Where .= " AND annual_revenue like '%".$_REQUEST['keyword']."%'";	
}
if(isset($_REQUEST['status']) && $_REQUEST['status']!='')
{
	$Where .= " AND status = '".$_REQUEST['status']."'";	
}
$Table		= "tbl_annual_revenue";
$Fields		= "id,annual_revenue,status,created_date";
...
//echo "<pre>";print_r($Cat);exit;
$smarty->display('manage-revenue.tpl');
?>