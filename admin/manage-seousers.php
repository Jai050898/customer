...
{	
	...
	{	
		...
		{
			$Idres = $Gen->GetSelWhere("tbl_seo_users",array("email"),"user_id = '".$id_arr[$i]."'");
		}
	}
	//echo "<pre>";print_r($_REQUEST);exit;
	$upar['status']	= $_REQUEST['hid_type'];
	$upid	= $Gen->UpdateQry('tbl_seo_users',$upar,"user_id IN(".$_REQUEST['hid_id'].")");
}
...
$Table		= "tbl_seo_users A LEFT JOIN tbl_country B ON A.country = B.Country_Code LEFT JOIN tbl_states C ON A.country = C.Country_Code AND A.state = C.State_ID";
...
//echo "<pre>";print_r($Users);exit;
$smarty->display('manage-seousers.tpl');
?>