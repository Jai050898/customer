...
{
	$Where .= " AND A.name like '%".$_REQUEST['keyword']."%' OR A.description like '%".$_REQUEST['keyword']."%'";	
}
if(isset($_REQUEST['status']) && $_REQUEST['status']!='')
{
	$Where .= " AND A.status = '".$_REQUEST['status']."'";	
}
$Table		= "tbl_projects A LEFT JOIN  tbl_tasks C ON A.project_id = C.project_id LEFT JOIN tbl_users D ON A.client_id = D.user_id";
$Fields		= "A.project_id,A.name,A.description,A.project_status,A.status,A.created_date,COUNT(C.task_id) as totaltasks,D.first_name,D.last_name";

$total		= $usr->TotalRows("tbl_projects A",$Where);
