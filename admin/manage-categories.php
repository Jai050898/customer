...
{	
	...
	{
		$res1 = $usr->SelectQuery("SELECT max(showorder) as maxid FROM `tbl_categories` WHERE `showorder` < '".$_REQUEST['order']."'");
		$res = $usr->SelectQuery("SELECT cat_id FROM `tbl_categories` WHERE showorder = '".$res1[0]['maxid']."'");
		...
		$usr->ExecQuery("UPDATE tbl_categories SET showorder = showorder-1 WHERE cat_id = '".$_REQUEST['id']."'");
		$usr->ExecQuery("UPDATE tbl_categories SET showorder = showorder+1 WHERE cat_id = '".$res[0]['cat_id']."'");
	}
	if($_REQUEST['act'] == "up")
	{
		//echo "SELECT min(showorder),cat_id FROM `tbl_categories` WHERE `showorder` > '".$_REQUEST['order']."'";
		$res1 = $usr->SelectQuery("SELECT min(showorder) as minid FROM `tbl_categories` WHERE `showorder` > '".$_REQUEST['order']."'");
		$res = $usr->SelectQuery("SELECT cat_id FROM `tbl_categories` WHERE showorder = '".$res1[0]['minid']."'");
...
{	
	...
	$upid	= $usr->UpdateQry('tbl_categories',$upar,"cat_id IN(".$_REQUEST['hid_id'].")");
}
...
//echo "<pre>";print_r($Cat);exit;
$smarty->display('manage-categories.tpl');
?>