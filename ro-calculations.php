<?php
   ...
   $page = 'customers';
    ...
    for($i=$currentYear; $i>=$lastYear; $i--) {
        ...
        $Fields = "SUM(laboramount) as laborSum, SUM(partsamount) as partsSum, SUM(hazardwasteamount) AS hazardSum, SUM(shopsuppliesamount) as shopSupplySum, SUM(taxamount) as taxSum, sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as grossSum, SUM(discountamount) as discountSum, SUM(manager_Charges) as managercharges";
        ...
        $roArray1    = array();
        $Where  = "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' AND (transaction_date between  DATE_FORMAT(NOW() ,'".$i."-01-01') AND DATE_FORMAT(NOW() ,'".$i."-%m-%d') )";
        $Fields = "SUM(laboramount) as laborSum, SUM(partsamount) as partsSum, SUM(hazardwasteamount) AS hazardSum, SUM(shopsuppliesamount) as shopSupplySum, SUM(taxamount) as taxSum, sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as grossSum, SUM(discountamount) as discountSum,SUM(manager_Charges) as managercharges";
