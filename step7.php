...
$smarty->assign('breadcrumb','Marketing Survey Step-7');
...
$marketingLeadersDetailarr = $usr->GetAllWhere("tbl_marketing_leaders","UserId = '".$_SESSION['User']['UID']."'");
if(isset($marketingLeadersDetailarr) && !empty($marketingLeadersDetailarr))
{
	...
	if(isset($marketingLeadersDetail['PurchaseList']) && $marketingLeadersDetail['PurchaseList'] != "")
		$smarty->assign('hidPurChaseMailing',"1");
	//echo "<pre>";print_r($marketingLeadersDetail);exit;
...
if(isset($_POST['btnSubmit']))
{
	...
	#set the values
	$customerMarketingLeaders['UserId'] = $_SESSION['User']['UID'];
	...
	if($_POST['isNew'])
	{
		$ins 	= $Gen->InsertQry('tbl_marketing_leaders',$customerMarketingLeaders);	
	}
	else
		$ins = $Gen->UpdateQry('tbl_marketing_leaders',$customerMarketingLeaders,"UserId = '".$_SESSION['User']['UID']."'");
