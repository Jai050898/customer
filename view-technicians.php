<?php
    ...
    $Where  = "1=1 AND A.company_id = '".$_SESSION['User']['xml_id']."' ";
        ...
        if(isset($_REQUEST['sdate']) && $_REQUEST['sdate'] != "" ) {
                  ...
                  $current_date = date('Y-m-d');
                    ...
                    $TotalWhr   .= "  AND B.transaction_date <= '".$current_date." 23:59:59'";
                    $Where1 = $Where.''.$TotalWhr."  GROUP BY A.technician ";
                    $total_temp =  $usr->GetSelWhere("XML_ro_details as A join XML_ro as B on A.ro_id=B.ro_id","A.technician",$Where1);
                    ...
                    $custAry = array();
                    $custAry = $usr->GetSelWhere("XML_ro_details as A join XML_ro as B on A.ro_id=B.ro_id","A.technician",$Where1);
                  ...
                  $spent_mtotal=0;
                        ...
                        for($i=0;$i<3;$i++){
                            ...
                            $TotalWhr0 .= $TotalWhr." AND B.transaction_date >='".$oneweek." 00:00:00'";
                            $sale_before_discount = "sum(A.laborhours)";
                            $Where3 = $Where."".$TotalWhr0."  AND A.technician = '".$cust."' GROUP BY A.technician";
                             $tech_weekdata   = $usr->GetSelWhere("XML_ro_details as A join XML_ro as B on A.ro_id=B.ro_id",$sale_before_discount." as total,A.technician",$Where3);
                             ...
                             $technician_data[$key]['total_week']=$spent_wtotal;
                                ...
                                $TotalWhr1 .= $TotalWhr." AND transaction_date >='".$onemnt." 00:00:00'";
                                $sale_before_discount = "sum(A.laborhours)";
                                $Where3 = $Where."".$TotalWhr1."  AND A.technician = '".$cust."' GROUP BY A.technician";
                            $tech_month  =$usr->GetSelWhere("XML_ro_details as A join XML_ro as B on A.ro_id=B.ro_id",$sale_before_discount." as total,A.technician",$Where3);
                                ...
                                $TotalWhr2 .= $TotalWhr." AND transaction_date >='".$oneday." 00:00:00'";
                                $sale_before_discount = "sum(A.laborhours)";
                                $Where3 = $Where."".$TotalWhr2."  AND A.technician = '".$cust."' GROUP BY A.technician";
                              $tech_daydata   = $usr->GetSelWhere("XML_ro_details as A join XML_ro as B on A.ro_id=B.ro_id",$sale_before_discount." as total,A.technician",$Where3);
                    ...
                    function GetSelWhere($tbl_name,$selFields,$where)
	{
        ...
        $smarty->assign('weekdate',$oneweek);
        $smarty->assign('daydate',$oneday);        
        $smarty->assign('month',$onemnt); 
        $smarty->assign('seldate',$current_date); 
