<?php
    ...
    // To Fetch Total ROws
    $Where  = "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' AND (reg_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 10 DAY) AND CURDATE())";
    $Table  = "XML_customers ";
    ...
    foreach($customersArray  as $key=>$value)
    {
        ...
        $Where3	= "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' and  cust_id = '".$value['cust_id']."' AND (transaction_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 10 DAY) AND CURDATE()) ORDER BY transaction_date ASC LIMIT 0,1";
