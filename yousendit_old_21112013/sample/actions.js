...
function refresh(divname){
	$('#viewform').load('http://www.marketingnavigator.info/yousendit/sample/ui.php #'+divname);
}

function refreshActions(divnameActions, divnameLink, divnameTitle){
		$('#actions').load('http://mm.autorepairmarketing.com/customer/yousendit/sample/ui.php #'+divnameActions);
		$('#link').load('http://mm.autorepairmarketing.com/customer/yousendit/sample/ui.php #'+divnameLink);
		$('#title').load('http://mm.autorepairmarketing.com/customer/yousendit/sample/ui.php #'+divnameTitle);		
}
...
function uploadForm(fileCount) {
		...
		$('#uploadf').ajaxSubmit({ 
		beforeSend: function(){
			$("#spinner").html('<img src="http://mm.autorepairmarketing.com/customer/yousendit/sample/images/roller.gif" alt="Wait" />');
		},
		success: function(data) {
			//alert(data);
			$("#spinner").html('');
				...
				error: function(){
					if (i<n) {
						...
						if (document.getElementById('method').value == 'preparesend'){
							dostuff('commitsend','http://mm.autorepairmarketing.com/customer/yousendit/sample/SendMethods.php',document.getElementById('token').value,document.getElementById('sendemailnotif').checked,document.getElementById('expiration').value,document.getElementById('bid1').value)
						}else {
							dostuff('commitupload','http://mm.autorepairmarketing.com/customer/yousendit/sample/StoreMethods.php',document.getElementById('token').value,document.getElementById('parent').value,document.getElementById('bid1').value)
						}
						}
				},
				success: function(){
