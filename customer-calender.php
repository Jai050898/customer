    ...
    for($i=0;$i<6;$i++) {
        for($j=0;$j<7;$j++) {
            ...
            if ($k<=$days) {
                ...
                $where_date = "company_id = '".$_SESSION['User']['xml_id']."' AND transaction_date > '".date($y.'-'.$m.'-'.$cal[$i][$j].' 00:00:00')."' AND transaction_date <= '".date($y.'-'.$m.'-'.$cal[$i][$j].' 23:59:59')."' GROUP BY cust_id,id";
   		//$where_date = "company_id = '".$_SESSION['User']['xml_id']."' AND DATE_FORMAT(transaction_date,'%Y-%m-%d') = '".date($y.'-'.$m.'-'.$cal[$i][$j])."' GROUP BY cust_id";
           ...
           //     echo "<pre>";print_r($dateSearchArr);
                ...
                if(!empty($dateSearchArr)){
                    ...
                    */
                    ...
                    $Where_cust = " company_id = '".$_SESSION['User']['xml_id']."' AND DATE_FORMAT(reg_date,'%Y-%m-%d') = '".date($y.'-'.$m.'-'.$cal[$i][$j])."' ";
