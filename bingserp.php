...
$smarty->assign('breadcrumb','Website - Bing SERP');
$usr 		= new General;
$Table		= "tbl_thumbnails";
$Fields		= "date";
$Where     = " customer_id = '".$_SESSION['User']['UID']."' AND type= 'B' GROUP BY date ";
...
//$Images = $Gen->GetSelWhere("tbl_thumbnails","date"," customer_id = '".$_SESSION['User']['UID']."' AND type= 'B' GROUP BY date ORDER BY date DESC");
for($i=0;$i<count($Images);$i++)
{
	$items = $Gen->GetSelWhere("tbl_thumbnails","image"," customer_id = '".$_SESSION['User']['UID']."'  AND type= 'B' AND date = '".$Images[$i]['date']."'");
