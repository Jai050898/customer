...
{
	header("Location: http://www.autorepairmarketing.com/customer-login.php");

}

 if(isset($_SESSION['User']['UID'])&& ($_SESSION['User']['is_staff']=='Y'))
{
	header("Location:http://www.autorepairmarketing.com/customer/customer-recent-customers_new_staff.php");
	    
}
	//echo "<pre>";print_r($_REQUEST);exit;
	$UsrDet				= $Gen->GetSelWhere('tbl_users','user_id,email,user_name,xml_id,password,company_name,Last_Login_Date,status,login_status,access_to_mark_survey,access_to_site_survey,access_to_integrated_survey,ismarketingsurveycompleted,iswebsurveycompleted,issurveycompleted,is_staff,created_date,attempts'," user_name = '".$_REQUEST['input_1']."' AND password = '".base64_encode($_REQUEST['input_2'])."'");
	$cnt			= count($UsrDet);
	$status = "F";
	...
	if($cnt != 0)
	{
		...
		{
			$Responce	= 'Account Locked, Please Try again After 10 hours.';
			$status = "L";
		}
		else
		{
