<?php
	...
	// To Fetch Total ROws
	$Where  = "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' AND (reg_date BETWEEN '".$fdate." 00:00:00' AND '".$tdate." 23:59:59') GROUP BY  source  ";
	...
	$sourceArray = array();
	$Where4	= " 1=1 AND company_id = '".$_SESSION['User']['xml_id']."' and  cust_id = '".$value['cust_id']."' AND (transaction_date BETWEEN '".$fdate." 00:00:00' AND '".$tdate." 23:59:59')  GROUP BY  source ORDER BY transaction_date ASC LIMIT 0,1 ";
	$sources         = "id, company_id, ro_id, cust_id,sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as gross , count(ro_id) as roCnt, source, referral ";

	$Totalcustomers	= $usr->TotalRows("XML_ro");

	$sourceArray	= $usr->GetSelWhere("XML_ro", $sources, $Where4);
	//$sourceArray    = 'gross' / 'roCnt' as 'averagero' ;

	//echo '<pre>';print_r($sourceArray);
	//$sourceArray    = 'gross' / 'roCnt' as 'averagero' ;


	if(!empty($sourceArray) && count($sourceArray) > 0){ 
	$allArray[$key]['id']   = $sourceArray['0']['id'];
	$allArray[$key]['ro_id']   = $sourceArray['0']['ro_id'];
	$allArray[$key]['company_id']   = $sourceArray['0']['company_id'];
	$allArray[$key]['cust_id']   = $sourceArray['0']['cust_id'];
