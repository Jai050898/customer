...
{
	$Fields 	= "project_id,name,description,project_status,status,client_id";
...
if(isset($_REQUEST['project_id']) && $_REQUEST['project_id'] != "" && $_REQUEST['hid_key']=='Post')
{
	...
	if($UpOverview)
	{
		...
		$chk = $Gen->GetSelWhere("tbl_project_status","project_status","project_id = '".$_REQUEST['project_id']."'");
	...
	header("Location:".SITEURL.'/client/manage-projects.php');
	exit;
}
...
if(isset($_REQUEST['hid_key']) && $_REQUEST['hid_key']=='Post')
{
	//echo '<pre>';print_r($_REQUEST);exit;
	$PrFields = $_REQUEST['Log'];
	$PrFields['client_id'] = $_SESSION['Client']['CID'];
