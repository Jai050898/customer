<?php
    ...
    for($i=$currentYear, $j =0; $i > $lastYear; $i--,$j++){
        ...
        $fields4 = "sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as grossSales, discountamount, count(id) as roCnt";
        $Table4	= "XML_ro ";
        ...
        $averageROB4Discount    = $totalROB4Discount / $grossSaleArray['0']['roCnt'];
        $allArray['averageROB4Discount'][$j] = $averageROB4Discount;


        // Total RO After Discount
        $roAfterDiscount    = $totalROB4Discount - $allArray['discount'][$j];
        $totalROAfterDiscount  = $totalROAfterDiscount + $roAfterDiscount;
        $averageROAfterDiscount = $totalROAfterDiscount / $grossSaleArray['0']['roCnt'];
