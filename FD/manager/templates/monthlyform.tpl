...
function sendajax()
{
	...
	$('#hid_type').val('Edit');
	$.post('{/literal}{$siteurl}{literal}/manager/monthlyformsubmit.php',{grosssales: $("#grosssales").val(), repairorders: $("#repairorders").val(), newcustomers: $("#newcustomers").val(), actualhours: $("#actualhours").val(), vibe: $("#vibe").val(), activity: $("#activity").val(), hid_type: $("#hid_type").val(), month: $("#month").val(), daysopen: $("#daysopen").val(), facebookfans: $("#facebookfans").val(), googleplususers: $("#googleplususers").val(), shopid: $("#shopid").val() }, function(data) {
	  if(data == "success")
	  {
