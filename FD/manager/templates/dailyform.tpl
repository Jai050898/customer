<div class="bodyleftpopup">
			...
			<input type="hidden" name="day" id="day" value="{$smarty.request.id}">
...
function sendajax()
{
	...
	$('#hid_type').val('Edit');
	$.post('{/literal}{$siteurl}{literal}/manager/dailyformsubmit.php',{grosssales: $("#grosssales").val(), repairorders: $("#repairorders").val(), newcustomers: $("#newcustomers").val(), actualhours: $("#actualhours").val(), vibe: $("#vibe").val(), activity: $("#activity").val(), hid_type: $("#hid_type").val(), day: $("#day").val(), shopid: $("#shopid").val() }, function(data) {
	//alert(data);
	  if(data == "success")
	  {
