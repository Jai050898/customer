...
//echo getcwd();exit;
//vars and config
define ('CSVVEHICLES', '/home/navigato/public_html/files/Year.csv');
set_time_limit(0);
$items = array ();
$i = 0;
$c = 0;
...
if (($handle = fopen(CSVVEHICLES, "r")) !== FALSE) { 
	...
	$num=1;
		...
		//Automobile values
		$values=array($VEHICLES[0], $VEHICLES[1]);
			
		
		unset($VEHICLES);
		
		//store sql into block for larger statements
		$ia = pre_insert_array($values, 2);
		...
		if (($c+1) % 500 == 0) {
			...
			//----------------------------------AUTOMOBILE				
			$t = "INSERT INTO MIS_year (MIS_Count, MIS_Year) VALUES ";
			...
			$t = $t . "
			ON DUPLICATE KEY UPDATE MIS_Count=VALUES(MIS_Count), MIS_Year=VALUES(MIS_Year);"; 
