...
$usr 		= new General;
$url = "http://www.autorepairmarketing.com/xml/FT_YOUR_EXTERNALID_HERE_2013-01-07-13-26-33.xml";
$ch = curl_init();
...
for($i=0;$i<count($array);$i++)
{
	...
	for($c=0;$c<count($array[$i]['Contact']);$c++)
	{
		...
		$insContact= $Gen->InsertQry('XML_customers',$PrFieldsContact);
	}
	...
	for($v=0;$v<count($array[$i]['Vehicle']);$v++)
	{
		...
		$insVehicle = $Gen->InsertQry('XML_vehicle',$PrFieldsVehicle);
	}
	...
	for($v=0;$v<count($array[$i]['ROHeader']);$v++)
	{
		...
		$insROHeader = $Gen->InsertQry('XML_ro',$PrFieldsROHeader);
	}
	...
	for($v=0;$v<count($array[$i]['RODetail']);$v++)
	{
		...
		$insRODetail = $Gen->InsertQry('XML_ro_details',$PrFieldsRODetail);
	}
	...
	for($v=0;$v<count($array[$i]['Schedule']);$v++)
	{
		...
		$insSchedule = $Gen->InsertQry('XML_schedule',$PrFieldsSchedule);
	}
}
?>