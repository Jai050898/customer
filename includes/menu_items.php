...
function recursiveMenu($menus,$parent = 0, $level = 0)
    ...
    if(isset($menu_permit) && $menu_permit != '')
    {
        ...
        $query = "SELECT U.*, (SELECT count(distinct U2.id) from user_menu U2 WHERE U2.parent_id = U.id) as ChildCount FROM user_menu U WHERE U.id IN ($menuids) order by menu_order ASC";
