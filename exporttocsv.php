    ...
    if(isset($_REQUEST['percentage']) && $_REQUEST['percentage']!='' && isset($_REQUEST['days']) && $_REQUEST['days']!='') {
        ...
        foreach($custArray as $key=>$cust){
            ...
            if(!empty($custDetAry)) {
                ...
                if($spent_total != '')
                    $customerArray[$key]['amountSpent']   = "$".number_format($spent_total, 2, ',', '.');
                else
                    $customerArray[$key]['amountSpent']   = "$0.00";
                if($customerArray[$key]['amountSpent'] != '' && $_REQUEST['percentage'] != '' ) {
                    $rebate = $customerArray[$key]['amountSpent'] * $_REQUEST['percentage'];
                    $customerArray[$key]['rebate']  = "$".number_format($rebate, 2, ',', '.');
                }
                else
