...
{
	$Cnt = $Gen->TotalRows('tbl_users',"user_name = '".$_REQUEST['val']."'  AND status IN ('A','I')");
	if($Cnt > 0)
		echo '1';
	else
		echo '0';
}
if(isset($_REQUEST['For']) && $_REQUEST['For'] == 'ChkUnameEdit')
{
	$Cnt = $Gen->TotalRows('tbl_users',"user_name = '".$_REQUEST['val']."' AND user_id != '".$_REQUEST['id']."' AND status IN ('A','I')");
	if($Cnt > 0)
		echo '1';
	else
		echo '0';
