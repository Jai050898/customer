<?php
    ...
    */
    ...
    for($i=$lastYear; $i<=$currentYear;$i++) {
        ...
        // To Calculate Total Customers Count
        $custTotalClass = '';
        $ctotalArray    = array();
        ...
        $Where3		= "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' AND YEAR(vreg_date) = '".$i."' GROUP BY vehicle_id";
        $fields3         = "vehicle_id";
        $Table3		= "XML_vehicle";
        $vtotalArray	= $usr->GetSelWhere($Table3, $fields3, $Where3);
    ...
    foreach($dataArray as $key=>$val) {
        ...
        $custVariance   = 0;
        if(isset($dataArray[$key+1]['custTotal']) && $dataArray[$key+1]['custTotal'] != 0){
            $custVariance    = abs($dataArray[$key]['custTotal'] / $dataArray[$key+1]['custTotal']);
            //echo "<prE>";print_r($custVariance);exit;
        }
        $custVariance   = abs($custVariance)*100;
        $dataArray[$key]['custVariance']   = $custVariance;
        
        
        // To Calculate RO's Variance
        $roVariance   = 0;
        if(isset($dataArray[$key+1]['roTotal']) && $dataArray[$key+1]['roTotal']){
            $roVariance    = abs($dataArray[$key]['roTotal'] / $dataArray[$key+1]['roTotal']);
        }
        $roVariance   = abs($roVariance)*100;
        $dataArray[$key]['roVariance']   = $roVariance;
        
        
        $dataArray[$key]['custTotalClass']    = '#000000';
        $dataArray[$key]['custVarianceClass']    = '#000000';
        $dataArray[$key]['roTotalClass']    = '#000000';
        $dataArray[$key]['roVarianceClass']    = '#000000';
        
         
        if($key != 0) {
            
            // To Get Customer Class (Red, Blue or Black)
            if($dataArray[$key]['custTotal'] > $dataArray[$key+1]['custTotal']){
                
                // To find Percentage Difference (should be blue only if the difference is Greater  thean 10%)
                $custPercDiff   = 0;
                if($dataArray[$key+1]['custTotal'] != 0){
                    $custPercDiff   = abs($dataArray[$key]['custTotal']/$dataArray[$key+1]['custTotal']);
                }
                
                if($custPercDiff > 10){
                    $dataArray[$key]['custVarianceClass']    = "#2090FF";
                }
            } elseif($dataArray[$key]['custTotal'] > $dataArray[$key+1]['custTotal']){
                $dataArray[$key]['custTotalClass']    = "#FF0000";
                $dataArray[$key]['custVarianceClass']    = "#FF0000";
            } else {
                $dataArray[$key]['custTotalClass']    = "#000000";
                $dataArray[$key]['custVarianceClass']    = "#000000";
            }
            ...
            if($dataArray[$key]['roTotal'] > $dataArray[$key+1]['roTotal']){
                // To find Percentage Difference (should be blue only if the difference is Greater  thean 10%)
                $roPercDiff   = 0;
                if($dataArray[$key+1]['roTotal'] != 0){
                    $roPercDiff   = abs($dataArray[$key]['roTotal']/$dataArray[$key+1]['roTotal']);
                }
                if($roPercDiff > 10){
                    $dataArray[$key]['roVarianceClass']    = "#2090FF";
                }
                
            } elseif($dataArray[$key]['roTotal'] > $dataArray[$key+1]['roTotal']){
    ...
    for($i=$currentYear,$j=1;$i>=$lastYear;$i--){
        ...
        //To get comparativegross salesclass(Red,Blue,Black)
if($i == $lastYear || $dataArray[$i]['custTotal'] == 0 || $i == $currentYear) {
            $dataArray[$i]['comparativeGrossSale'] = 0;
        } else {
            $dataArray[$i+1]['comparativeGrossSale'] = $dataArray[$i+1]['grossSale'] - $dataArray[$i]['grossSale'];
        ...
        // to get varaiance class (Red,blue,black)
        $dataArray[$i+1]['comparativeGrossSaleClass']    = '#000000';
        $dataArray[$i+1]['varianceClass']   = '#000000';
        if($i != $currentYear){
            if($dataArray[$i]['comparativeGrossSale'] < $dataArray[$i+1]['comparativeGrossSale']){
    ...
    $smarty->display('customer-dc1.tpl');
?>
