    ...
    for($i=$endingYear; $i <= $currentYear; $i++) {
        ...
        for($j=1;$j<=12;$j++) {
            ...
            for($k=1;$k<=$maxday;$k++) {
                ...
                $roArray    = array();
                $roArray    = $Gen->GetSelWhere('XML_ro', 'sum(laboramount+partsamount+taxamount+hazardwasteamount+shopsuppliesamount) as gross', "1=1 AND company_id = '".$_SESSION['User']['xml_id']."' AND transaction_date = '".$monthArray[$k]."'");
                if(count($roArray) > 0 && $roArray[0]['gross']){
                    
                }
                $monthArray[$k] = $roArray;
