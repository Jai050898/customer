...
$smarty->assign('breadcrumb','Website - Google SERP');
$usr 		= new General;
$Table		= "tbl_thumbnails";
$Fields		= "date";
$Where     = " customer_id = '".$_SESSION['User']['UID']."' AND type= 'G' GROUP BY date ";
...
//$Images = $Gen->GetSelWhere("tbl_thumbnails","date"," customer_id = '".$_SESSION['User']['UID']."' AND type= 'G' GROUP BY date ORDER BY date DESC");
for($i=0;$i<count($Images);$i++)
{
	$items = $Gen->GetSelWhere("tbl_thumbnails","image"," customer_id = '".$_SESSION['User']['UID']."'  AND type= 'G' AND date = '".$Images[$i]['date']."'");
